...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, h=0, w=0, eta=8, stride=1, base_width=64, dilation=1):
        ...
        if self.dynamic:
            ...
            self.height_2 = conv2d_out_dim(h, 3, dilation, 2, dilation)
            self.width_2 = conv2d_out_dim(w, 3, dilation, 2, dilation)
            self.mask_s = Mask_s(self.height_2, self.width_2, inplanes, eta, eta)  
            self.upsample_1 = nn.Upsample(size=(self.height_2, self.width_2), mode='nearest')
            self.upsample_2 = nn.Upsample(size=(self.height_2, self.width_2), mode='nearest')

            self.mask_c1 = Mask_c(inplanes, width)
            self.mask_c2 = Mask_c(width, width)        
    ...
    def forward(self, input: torch.Tensor):
        if self.dynamic:        
            x, txt_emb, norm_1,  norm_2, flops = input    
            # spatial mask
            mask_s_m, norm_s, norm_s_t = self.mask_s(x) # [N, 1, h, w]
            mask_c1, norm_c1, norm_c1_t = self.mask_c1(x, txt_emb)
        ...
        if self.dynamic: 
            out * mask_c1 * mask_s1  if not self.training else out * mask_c1 * mask_s1  
            mask_c2, norm_c2, norm_c2_t = self.mask_c2(out, txt_emb)
            
        out = self.relu2(self.bn2(self.conv2(out)))
        
        if self.dynamic: 
            out = out * mask_c2* mask_s if not self.training else out * mask_c2 * mask_s
        ...
        if self.dynamic: 
            ...
            return (out, txt_emb, norm_1, norm_2, flops)
        return out
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, h=75, w=133, input_resolution=224, width=64):
        super().__init__()
        ...
        self.layer4, h, w = self._make_layer_dynamic(width * 8, layers[3], 14*2, 14*2, stride=2)
    ...
    def _make_layer(self, planes, blocks, stride=1):
        ...
        layers = [Bottleneck(self._inplanes, planes,0,0,8, stride)]        
        self._inplanes = planes * Bottleneck.expansion
        for _ in range(1, blocks):
            layers.append(Bottleneck(self._inplanes, planes))
        return nn.Sequential(*layers)
    
    def _make_layer_dynamic(self, planes, blocks, h, w, stride=1):

        layers = [Bottleneck(self._inplanes, planes, h, w, 8, stride)]

        h = conv2d_out_dim(h, kernel_size=1, stride=stride, padding=0)
        w = conv2d_out_dim(w, kernel_size=1, stride=stride, padding=0)             
        self._inplanes = planes * Bottleneck.expansion
        for _ in range(1, blocks):
            layers.append(Bottleneck(self._inplanes, planes, h, w))
